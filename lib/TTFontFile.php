...
class TTFontFile {
	...
	function getMetrics($file) {
		...
		if ($this->fh===false)
			throw new PdfException('Can\'t open file ' . $file);
		$this->_pos = 0;
		$this->charWidths = '';
		$this->glyphPos = array();
		...
		if ($version==0x4F54544F) 
			throw new PdfException("Postscript outlines are not supported");
		if ($version==0x74746366) 
			throw new PdfException("ERROR - TrueType Fonts Collections not supported");
		if (!in_array($version, array(0x00010000,0x74727565)))
			throw new PdfException("Not a TrueType font: version=".$version);
	...
	function get_table($tag) {
		...
		if ($length == 0) { throw new PdfException('Truetype font ('.$this->filename.'): error reading table: '.$tag); }
		fseek($this->fh,$pos);
		return (fread($this->fh,$length));
	}
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	function extractInfo() {
		...
		///////////////////////////////////
			...
			if ($format != 0)
				throw new PdfException("Unknown name table format ".$format);
			...
			for ($i=0;$i<$numRecords; $i++) {
				...
				if ($platformId == 3 && $encodingId == 1 && $languageId == 0x409) { // Microsoft, Unicode, US English, PS Name
					...
					if ($length % 2 != 0)
						throw new PdfException("PostScript name is UTF-16BE string of odd length");
					$length /= 2;
					$N = '';
					while ($length > 0) {
			...
			if ($names[6])
				...
				$psName = '';
			if (!$psName)
				throw new PdfException("Could not find PostScript font name");
			$this->name = $psName;
		...
		if ($glyphDataFormat != 0)
			throw new PdfException('Unknown glyph data format '.$glyphDataFormat);
		...
		if (isset($this->tables["OS/2"])) {
			...
			if ($fsType == 0x0002 || ($fsType & 0x0300) != 0) {
				throw new PdfException('ERROR - Font file '.$this->filename.' cannot be embedded due to copyright restrictions.');
		...
		if ($metricDataFormat != 0)
			throw new PdfException('Unknown horizontal metric data format '.$metricDataFormat);
		$numberOfHMetrics = $this->read_ushort();
		if ($numberOfHMetrics == 0) 
			throw new PdfException('Number of horizontal metrics is 0');
		...
		if (!$unicode_cmap_offset)
			throw new PdfException('Font ('.$this->filename .') does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)');
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	function makeSubset($file, &$subset) {
		...
		$glyphData = ($glyfLength < $this->maxStrLenRead) ? $glyphData = $this->get_table('glyf') : '';
		...
		foreach($subsetglyphs AS $originalGlyphIdx => $uni) {
			...
			if ($glyfLength < $this->maxStrLenRead) {
				...
				$data = $glyphLen > 0 ? $this->get_chunk($glyfOffset+$glyphPos,$glyphLen) : '';
			}

			$up = $glyphLen > 0 ? unpack("n", substr($data,0,2)) : [];
	...
	function getLOCA($indexToLocFormat, $numGlyphs) {
		...
		if ($indexToLocFormat == 0) {
			...
			throw new PdfException('Unknown location table format '.$indexToLocFormat);
	}
